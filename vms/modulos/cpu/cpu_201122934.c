...
// Para los porcentajes de RAM se debe dividir entre 100
// porque se multiplico por 10000 asi se obtiene más detalles del %
...
static int escribir_a_proc(struct seq_file *file_proc, void *v)
{
    ...
    #ifndef CONFIG_MMU
        pr_err("Sin MMU, no se puede calcular RSS.\n");
        return -EINVAL;
    #endif
    ...
    seq_printf(file_proc, "\"cpu_porcentaje_en_uso\":%lu,\n", cpu_percentage);
    ...
    for_each_process(task)
    {
        ...
        // int porcentaje = ((rss/4096) / (total_ram_pages))*100;
        unsigned long porcentaje = (rss * 10000ULL) / (total_ram_pages * 4096ULL);
        // seq_printf(file_proc, "\"total_ram_pages\":%d,\n", total_ram_pages);
...
{
    proc_create("cpu_201122934", 0, NULL, &archivo_operaciones);
